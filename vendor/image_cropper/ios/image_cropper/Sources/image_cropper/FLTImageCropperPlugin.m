...
@interface FLTAdjustedCropViewController : TOCropViewController
@end

@implementation FLTAdjustedCropViewController

- (UIView *)flt_findToolbarInView:(UIView *)view {
    Class toolbarClass = NSClassFromString(@"TOCropToolbar");
    if (!toolbarClass) return nil;
    if ([view isKindOfClass:toolbarClass]) return view;

    for (UIView *subview in view.subviews) {
        UIView *match = [self flt_findToolbarInView:subview];
...
- (void)viewDidLayoutSubviews {
    ...
    UIView *toolbar = [self flt_findToolbarInView:self.view];
    if (!toolbar) return;
    ...
    CGFloat targetMinY = CGRectGetHeight(self.view.bounds) - safeBottom - CGRectGetHeight(toolbarFrame) - 12.0f;

    if (targetMinY < toolbarFrame.origin.y) {
        toolbarFrame.origin.y = targetMinY;
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if ([@"cropImage" isEqualToString:call.method]) {
        ...
        FLTAdjustedCropViewController *cropViewController;

        if ([@"circle" isEqualToString:cropStyle]) {
            cropViewController = [[FLTAdjustedCropViewController alloc] initWithCroppingStyle:TOCropViewCroppingStyleCircular image:image];
        } else {
            cropViewController = [[FLTAdjustedCropViewController alloc] initWithImage:image];
        }
