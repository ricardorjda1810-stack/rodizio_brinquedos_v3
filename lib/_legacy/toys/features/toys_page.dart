...
class _ToysPageState extends State<ToysPage> {
  ...
  Widget _buildLeadingPhoto(String? photoPath) {
    ...
    // Mantém tamanho fixo e seguro pro ListTile (não “estoura”).
    return ClipRRect(
  ...
  PopupMenuButton<ToysSortMode> _buildSortMenu(ToysState state) {
    return PopupMenuButton<ToysSortMode>(
      tooltip: 'Ordenar',
      initialValue: state.sortMode,
      onSelected: (mode) => _controller.setSortMode(mode),
      itemBuilder: (context) => [
        CheckedPopupMenuItem(
          value: ToysSortMode.createdDesc,
          checked: state.sortMode == ToysSortMode.createdDesc,
          child: Text(toysSortModeLabel(ToysSortMode.createdDesc)),
        ),
        CheckedPopupMenuItem(
          value: ToysSortMode.createdAsc,
          checked: state.sortMode == ToysSortMode.createdAsc,
          child: Text(toysSortModeLabel(ToysSortMode.createdAsc)),
        ),
        const PopupMenuDivider(),
        CheckedPopupMenuItem(
          value: ToysSortMode.nameAsc,
          checked: state.sortMode == ToysSortMode.nameAsc,
          child: Text(toysSortModeLabel(ToysSortMode.nameAsc)),
        ),
        CheckedPopupMenuItem(
          value: ToysSortMode.nameDesc,
          checked: state.sortMode == ToysSortMode.nameDesc,
          child: Text(toysSortModeLabel(ToysSortMode.nameDesc)),
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<ToysState>(
      ...
      builder: (context, snap) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text('Brinquedos [UI/toys/toys_page.dart]'),
            actions: [
